export class JsonHelper {
     ...
     */
    static UpdateNode<T>(obj: T, path: string, value, createNodes?: boolean): T {
        try {
    ...
    static GetLeafes(obj: any, path?: string): string[] {
        try {
            let leafes: string[] = [];
            if (path === undefined) {
                path = '';
    ...
    static Merge<T>(ob1: T, obj2): T {
        try {
            ...
            const leafes = JsonHelper.GetLeafes(obj2);

            leafes.forEach(leaf => {
                ob1 = JsonHelper.UpdateNode<T>(
                    ob1,
                    leaf,
                    JsonHelper.GetValue(obj2, leaf),
                    true
                );
            });
